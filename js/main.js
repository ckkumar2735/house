        ...
        jQuery(document).ready(function ($)
            {
                ...
                $('#home_page_slider_01').slick({
                    ...
                    dots:true,
                    swipe:false,
                    prevArrow: '<div class="home_custom_arrow prev_arrow"><span></span></div>',
                    nextArrow: '<div class="home_custom_arrow next_arrow"><span></span></div>',
                    
                });

                $.validator.addMethod("lead_emailid", function(value, element) {
                    // allow any non-whitespace characters as the host part
                    var testEmail = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,4}$/i;
                    return this.optional( element ) || testEmail.test( value );
                }, 'Please enter a valid email address');

                $("#evolve-form").validate(
                    {
                        //validation rules
                        rules: {
                            evolve_name : {
                        ...
                        {
                            // form.preventDefault();
                            // return false;var str = $( "form" ).serialize();
                            $('#evolve-form-btn').attr('disabled','disabled');
                            $('#form-description').html('Please Wait...');
                            $.ajax({
                                url: base_url + "AjaxController/saveEvolveForm",
                                type: "POST",
                                data: $("#evolve-form").serialize(),
                                success: function (response)
                                {
                                    var returnedData = JSON.parse(response);
                                    if (returnedData.status)
                                    {
                                        // $('#form-description').html(returnedData.msg);
                                        // setTimeout(function()
                                        // {
                                        //     location.reload(true);
                                        // }, 6000);
                                        $(location).attr('href', base_url+'thankyou')
                                    }
                                    else
                                    {
                                        $('#evolve-form-btn').removeAttr('disabled');
                                        $('#form-description').html(returnedData.msg);
                                    }
                                },
                                ...
                                {
                                    $('#evolve-form-btn').removeAttr('disabled');
                                    $('#form-description').html("Something went wrong,<br> Please try again.");
                                }
                            });
                        }
                    });
          ...
          jQuery.validator.addMethod("lettersonly", function(value, element) {
                            ...
                            }, "This Field containt Letters only"); 
...
////////// Contactusform Ajax /////////
        ...
        jQuery(function () {
            $('#notification_checkbox').click(function (e) {
                // e.preventDefault();
                // alert();
                // $("#get_value").val('1');
            ...
            $("#contact-form").validate(
              
                {
                    ...
                    submitHandler: function(form)
                    {
                        ...
                        $('#form_messege_1').html('Please Wait...');

                    var selected = [];
         
                    //Reference the CheckBoxes and insert the checked CheckBox value in Array.
                    $.each($("input[name='roles']:checked"),function () {
                        selected.push($(this).attr('data-val'));
                        // alert(selected);
                    });
                    ...
                    // var cb=$("input[name='check_box']:checked").attr('data-val');
                    var cb = $("#notification_checkbox").val();  
                    var notification = "";               
                    if ($('#notification_checkbox').is(":checked")){
                        // var notification = $("#notification_checkbox").attr('data-val');
                        notification = "yes"
                        // alert(notification);
                    }
                    else{
                     ...
                     if (cb==1) {
                        ...
                        check_box = "";
                          // $('#form_messege_1').html("<p style='color:red;'>Please select at-least one Checkbox.</p>");
                          $('#form_messege_1').html('Please Wait...');
                          
                     }
                        $.ajax({
                            url: base_url + "AjaxController/saveContactUSForm",
                            type: "POST",
                            data: {'name':name,'lastname':lastname,'email':email,'phone_number':phone_number,'job_title':job_title,'company_name':company_name,'sector':sector,'country':country,'contact_message':contact_message,'myJsonString':myJsonString,'notification':notification},
                            success: function (response)
                            {
                                var returnedData = JSON.parse(response);
                                if (returnedData.status)
                                {
                                    $('#form_messege_1').html('Please Wait...');
                                    // $('#form_messege_1').html(returnedData.msg);
                                    $(location).attr('href', base_url+'thankyou')
                                    // setTimeout(function()
                                    // {
                                    //     // location.load('thankyout.php');
                                    // }, 1000);
                                }
                                else
                                {
                            ...
                            {
                                $('#contact-form-btn').removeAttr('disabled');
                                $('#form_messege_1').html("<p style='color:red;'>Something went wrong,<br> Please try again.</p>");
                            }
                        });
                    }else{
                         $('#contact-form-btn').removeAttr('disabled');
                         // $('#form_messege_1').html(returnedData.msg);
                        $('#form_messege_1').html("<a href='#services'><p style='color:red;'>Please select at-least one Checkbox.</p></a>");
            ...
            $(function () {
                $("#contact-form-btn").click(function () {
                   ...
                   var isValid = checked > 0;
                    ...
                    $('#form_messege_1').show().html("<a href='#services'><p style='color:red;'>Please select at-least one Checkbox.</p><a>");  
                });
            });
        }); 
    jQuery.validator.addMethod("lettersonly", function(value, element) {
...
//////   Careee Form Ajax  ///////
        $(document).ready(function(){
            ...
            $('option:selected', '#designation').removeAttr('selected');
            var job_title = $(this).attr('value');
            $('#designation option[value="'+job_title+'"]').attr("selected", "selected");

            });
        });
        $('#carrer-form-btn').click(function(e){
          e.preventDefault();
        ...
        // File Validation
          var file_value = $("#career_file").val();
          var file_extention = file_value.substring(file_value.lastIndexOf('.') + 1);


            if (file_value!=""){
                var file_size = document.getElementById("career_file").files[0];
                // alert(file_size.size);
            }

            if (file_value==""){
                $("#file_error_mssg").show().text("Please select file");
            }
            else if (file_extention == "doc" || file_extention == "docx" || file_extention == "pdf" ){
                if (file_size.size > 2000000) {
                    $("#file_error_mssg").show().text("File limit should be less then 2 MB");
                }
                else{
                    $("#file_error_mssg").hide();
                }
            }
            else{
                $("#file_error_mssg").show().text("Please select pdf,doc,docx");
            }
          // File Validation
            jQuery.validator.addMethod("lettersonly", function(value, element) {
                            return this.optional(element) || /^[a-zA-Z\s]+$/i.test(value);
            ...
            var isapplicationval = $("#career_form").valid();
            if(isapplicationval){
                        var firstname = $("#career_firstname").val();
                        var lastname = $("#career_lastname").val();
                        var phone_number = $("#career_phone").val();
                        var email = $("#career_email").val();
                        var designation = $("#designation").val();
                        var message = $("#career_message").val();
                        var file_val = $('#career_file').prop('files')[0];
                        var filedata = $("#career_file").val();
                        if (filedata != "") {
                        var fileName = file_val.name;
                    ...
                    form_data.append('career_file', file_data);
                    form_data.append('data', Jdata);
                    // alert(form_data);
                    $(".loader").show();
                    $.ajax({

                    url: base_url + "AjaxController/jobAppForm",
                    datatype:"JSON",
                    contentType: false,
                         ...
                         result =  JSON.parse(response);
                           var returnedData = JSON.parse(response);
                            ...
                            document.getElementById('career_form').reset();
                             $(".loader").hide();
                            // $('#job_application_success_msg').html('Thank you for your application. We will get back to you if you are shortlisted.');
                            // setTimeout(function() {
                            // $('#job_application_success_msg').html(" ");
                            //   }, 6000 );
